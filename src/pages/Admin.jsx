...
export default function Admin(){
  ...
  const save = async () => {
    ...
    setStaff(next); setStatus('Guardando...')
    const data = await loadAll(); await saveAll({ ...data, staff: next })
    setStatus('Guardado ✅'); setTimeout(()=>setStatus(''), 2000); setForm(blank)
  }
  const remove = async (id) => {
    const next = staff.filter(s=>s.id!==id); setStaff(next)
    const data = await loadAll(); await saveAll({ ...data, staff: next })
  }
  return (
    <section className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-sky-700">Administración</h1><p className="text-sm text-gray-600">Demo editable (se guarda en este navegador).</p></div>
        <span className="text-sm text-gray-600">{status}</span>
      </div>
      <div className="grid md:grid-cols-2 gap-6">
        <div className="bg-white rounded-2xl shadow p-4 space-y-2">
          <h2 className="font-semibold">Agregar / Editar personal</h2>
